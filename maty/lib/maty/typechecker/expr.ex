defmodule Maty.Typechecker.Expr do
  @moduledoc """
  ...
  defp get_type_internal({variable, _, args}, env) when is_atom(variable) and is_atom(args) do
    case Map.fetch(env[:variable_ctx], variable) do
      {:ok, type} -> type
      :error -> :error
    end
  end
      ...
      case new_vars[:error] do
        ...
        message ->
          ...
          {:error,
           message <>
             ": Expected " <>
             Enum.join(param_type_list, ", ") <> ", but found " <> Macro.to_string(params)}
      end
    end
  end
  ...
  def get_vars([{:|, _, [operand1, operand2]}], {:list, type}),
    do: [get_vars(operand1, type), get_vars(operand2, {:list, type})]

  def get_vars({:|, _, [operand1, operand2]}, {:list, type}),
    do: [get_vars(operand1, type), get_vars(operand2, {:list, type})]

  def get_vars(op, {:list, type}) when is_list(op),
  ...
  def get_vars(ops, {:tuple, type_list}) when is_tuple(ops),
